...
<div class="flex flex-col h-full w-full justify-center items-center text-red-500 absolute z-10 transition-width duration-300">
  <!-- Audio elements -->
  ...
  <div class="h-full w-full fixed top-0 bg-black">
    ...
    <div class={`justify-center items-center flex-col h-full m-0 flex ${isSinglePlayerMatch ? 'twisted-ui-s' : 'twisted-ui'}`}>
      <div class={`flex-1 ${isSinglePlayerMatch ? 'h-full' : 'h-1/2'} w-full bg-transparent ${isSinglePlayerMatch ? '' : 'animated-left'}`} 
           style="background: radial-gradient(at bottom, #00F0FF 0%, #1C62CB 60%);"></div>
      {#if !isSinglePlayerMatch}
        <div class="flex-1 h-1/2 w-full bg-transparent animated-right" 
             style="border: 12px solid black; background: radial-gradient(at top, #FF981E 0%, #CB1E47 60%);"></div>
      {/if}
    </div>
    ...
    <div class="max-h-screen max-w-full overflow-hidden h-full w-full fixed left-0 top-0 bottom-0 right-0">
      <div>
        <div class="absolute left-0 right-0 top-0 bottom-0 bg-cover bg-center" 
             style={`background-image: url("${polkaDots}")`}></div>
        <div class="absolute left-0 right-0 top-0 bottom-0 bg-cover bg-center transition-transform duration-0" 
             style={`background-image: url("${spikes}"); transform: scale(${scale}); transform-origin: 50% 50%;`}></div>
      </div>
    </div>
    
    <!-- Content layer -->
    <div class="justify-center items-center flex-col h-full m-0 flex absolute top-0 bottom-0 right-0 left-0 overflow-hidden h-full max-h-full">
      <div class="justify-center items-center flex-col h-full m-0 flex w-full max-w-5xl justify-between">
        ...
        <div class="relative w-full flex justify-center mt-10">
          <div class="relative bg-contain bg-center bg-no-repeat h-[85px] w-[430px]" style={`background-image: url("${singlePlayerBanner}")`}>
            <!-- Crossed swords at left side -->
            <div class="absolute left-[110px] -ml-20 top-1/2 -translate-y-1/2 z-10">
              <img src={sword1} alt="Sword 1" class="absolute h-[70px] w-[70px] transform -rotate-8 z-20" bind:this={refSw1} />
              <img src={sword2} alt="Sword 2" class="relative h-[70px] w-[70px] transform rotate-8 z-20 top-0" bind:this={refSw2} />
            </div>
            <div class="absolute w-full h-full flex items-center justify-center">
              ...
              {:else}
                <div class="mode-text flex gap-2 items-center">
                  <span class="text-white font-bold text-xl">Challenge Mode in</span>
        ...
        <div class="mt-6 mb-6 w-full max-w-4xl">
          <div class={`flex ${isSinglePlayerMatch ? 'flex-col items-center' : 'flex-row items-center justify-center'} gap-8`}>
