...
<div
  class="w-full bg-white rounded-3xl shadow-md"
  style="clip-path: inset(0 0 0 0 round 24px);"
>
  <!-- Header -->
  ...
  <div>
    {#if isLoading}
      {#each Array(5) as _, rowIndex}
        <div
          class="flex first:bg-blue-100 even:bg-gray-100 animate-pulse top-[40px] z-10"
        >
          {#each columns as column}
            <div class="px-2 md:px-4 py-2" style="width: {column.width}">
              {#if column.type === "avatar"}
                <div
                  class="inline-block w-6 h-6 md:w-8 md:h-8 bg-gray-300 rounded-full mr-2"
                ></div>
              {/if}
    ...
    {:else}
      {#each data as row, i}
        <div class="flex first:bg-[var(--primary-color)] even:bg-gray-100">
          {#each columns as column}
            <div
              class="px-1 md:px-4 py-2 text-left flex items-center"
              style="width: {column.width}"
            >
              <div class="flex items-center gap-2 w-full">
                ...
                {:else if column.key === "name"}
                  ...
                  <div
                    class="truncate font-fredoka font-medium text-sm md:text-base text-gray-600"
                  >
                    {row[column.key]}
                  </div>
